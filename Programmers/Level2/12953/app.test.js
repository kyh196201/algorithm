...
function solutionReduce(numbers = []) {
  ...
  return numbers.reduce((lcm, cur) => getLcm(lcm, cur), numbers[0]);
}
...
describe('N개의 최소공배수', () => {
  it('test', () => {
    ...
    // reduce를 이용한 풀이
    expect(solutionReduce([100])).toBe(100);
    expect(solutionReduce([2, 6, 8, 14])).toBe(168);
    expect(solutionReduce([1, 2, 3])).toBe(6);
  });
});
